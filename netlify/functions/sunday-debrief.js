...
exports.handler = async function(event, context) {
  try {
    ...
    const agentStats = AGENTS.map(name => {
      ...
      const activeListings = rows[rows.length - 1]?.active_listings || 0;
      const activeBuyers = rows[rows.length - 1]?.active_buyers || 0;
      const closings = rows[rows.length - 1]?.closings || 0;
      const leadToAppt = rows[rows.length - 1]?.leads > 0
        ? Math.round((rows[rows.length - 1]?.appt_held / rows[rows.length - 1]?.leads) * 100)
        : 0;

      const metCalls = avgCalls >= MINS.calls;
      const metConvos = avgConvos >= MINS.convos;
      const metFollowups = avgFollowups >= MINS.followups;
      const metRefs = avgRefs >= MINS.refs;
      const minimumsScore = [metCalls, metConvos, metFollowups, metRefs].filter(Boolean).length;

      return {
        name, rows, daysSubmitted, storiesDone,
        avgCalls, avgConvos, avgFollowups, avgRefs,
        totalAppts, activeListings, activeBuyers, closings,
        leadToAppt, minimumsScore, metCalls, metConvos, metFollowups, metRefs,
        noData: false
      };
    });
...
- Avg daily calls: ${s.avgCalls} (min: 20) — ${s.metCalls ? '✓ MET' : '✗ MISSED'}
- Avg daily convos: ${s.avgConvos} (min: 5) — ${s.metConvos ? '✓ MET' : '✗ MISSED'}
- Avg daily follow-ups: ${s.avgFollowups} (min: 5) — ${s.metFollowups ? '✓ MET' : '✗ MISSED'}
- Avg daily referrals asked: ${s.avgRefs} (min: 5) — ${s.metRefs ? '✓ MET' : '✗ MISSED'}
- Minimums hit: ${s.minimumsScore}/4
- Stories posted: ${s.storiesDone}/7 days
- Appointments set this week: ${s.totalAppts}
...
This report goes to Kevin and Steven (team leads). Be direct, data-driven, and honest. Write like a coach who genuinely cares about each agent's success — not corporate, not fluffy, but real.

Here is this week's data for each agent:
...
2. One section per agent with:
   - Their name as a header
   - What they crushed
   - What they missed and by how much
   - Any concerning patterns (if this is the 2nd or 3rd week missing something, flag it hard)
   - ONE specific Monday morning action item
3. MONDAY MORNING PRIORITIES — a ranked list of who needs attention first and why
  ...
  <div style="max-width:640px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">
    ...
    <div style="padding:32px;font-size:15px;line-height:1.8;color:#1a1a2e">
      ${debrief.replace(/\n\n/g, '</p><p>').replace(/\n/g, '<br>').replace(/^/, '<p>').replace(/$/, '</p>')
        ...
        .replace(/#{1,3} (.*?)(<br>|<\/p>)/g, '<h2 style="font-family:Georgia,serif;color:#0f2557;font-size:18px;margin:24px 0 8px;border-bottom:2px solid #c9a84c;padding-bottom:6px">$1</h2>')}
    </div>
    <div style="background:#f4f5f9;padding:20px 32px;text-align:center;font-size:12px;color:#888">
      Sonder Legacy CRM · sonderlegacy.org<br>
      This report was generated automatically every Sunday at 5PM MT
    </div>
  </div>
</body>
</html>`;
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ success: true, message: 'Debrief sent successfully' })
    };
